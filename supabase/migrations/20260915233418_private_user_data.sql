...
create trigger saved_comparisons_set_updated_at before update on public.saved_comparisons for each row execute function public.set_updated_at();
...
as $$
begin
  insert into public.profiles (id, display_name) values (new.id, nullif(trim(new.raw_user_meta_data ->> 'display_name'), '')) on conflict (id) do nothing;
...
create policy "preferences_select_own" on public.preferences for select to authenticated using (user_id = (select auth.uid()));
drop policy if exists "preferences_insert_own" on public.preferences;
create policy "preferences_insert_own" on public.preferences for insert to authenticated with check (user_id = (select auth.uid()));
drop policy if exists "preferences_update_own" on public.preferences;
create policy "preferences_update_own" on public.preferences for update to authenticated using (user_id = (select auth.uid())) with check (user_id = (select auth.uid()));
...
create policy "favorites_update_own" on public.favorites for update to authenticated using (user_id = (select auth.uid())) with check (user_id = (select auth.uid()));
...
create policy "saved_comparisons_select_own" on public.saved_comparisons for select to authenticated using (user_id = (select auth.uid()));
drop policy if exists "saved_comparisons_insert_own" on public.saved_comparisons;
create policy "saved_comparisons_insert_own" on public.saved_comparisons for insert to authenticated with check (user_id = (select auth.uid()));
drop policy if exists "saved_comparisons_update_own" on public.saved_comparisons;
create policy "saved_comparisons_update_own" on public.saved_comparisons for update to authenticated using (user_id = (select auth.uid())) with check (user_id = (select auth.uid()));
drop policy if exists "saved_comparisons_delete_own" on public.saved_comparisons;
create policy "saved_comparisons_delete_own" on public.saved_comparisons for delete to authenticated using (user_id = (select auth.uid()));
