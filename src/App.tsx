...
function App() {
  ...
  const navigate = (next: 'dashboard' | 'explore') => { setView(next); window.location.hash = next }
  return <div className="app-shell"><header className="topbar"><button className="brand brand-button" onClick={() => navigate('dashboard')} aria-label="RaceMetrics início"><span className="brand-mark">RM</span><span>RaceMetrics</span></button><nav className="nav" aria-label="Navegação principal"><button className={view === 'dashboard' ? 'active' : ''} onClick={() => navigate('dashboard')}>Dashboard</button><button className={view === 'explore' ? 'active' : ''} onClick={() => navigate('explore')}>Explore</button><button onClick={() => { window.location.hash = 'compare' }}>Compare</button><button onClick={() => { window.location.hash = 'analytics' }}>Analytics</button></nav><div className="topbar-actions"><button className="icon-button" aria-label="Pesquisar" onClick={() => navigate('explore')}><Search size={18} /></button><button className="profile-button" title={user?.email ?? 'Conta'}><CircleUserRound size={18} /><span>{displayName}</span></button><button className="icon-button" aria-label="Sair" onClick={() => void signOut()}><LogOut size={17} /></button></div></header>{view === 'dashboard' ? <Dashboard /> : <Explore />}</div>
}
